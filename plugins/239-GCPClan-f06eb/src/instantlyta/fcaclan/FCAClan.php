                ...
                switch ($args[0]) {
                    case "help":
                        $help = [
                            ...
                            "§l§cClan §f↣ §e/c : §achat kênh Clan",
                            "§l§cClan §f↣ §e/clan top : §axem xếp hạng Clan trong server",
                            "§l§cClan §f↣ §e/clan create : §atạo Clan mới",
                            "§l§cClan §f↣ §e/clan delete : §axoá Clan của bạn",
                            "§l§cClan §f↣ §e/clan join : §agửi đơn xin gia nhập Clan",
                            "§l§cClan §f↣ §e/clan quit : §athoát Clan của bạn",
                            "§l§cClan §f↣ §e/clan accept : §achấp nhận lời mời vào Clan",
                            "§l§cClan §f↣ §e/clan decline : §atừ chối lời mời vào Clan",
                            "§l§cClan §f↣ §e/clan donate : §acống hiến cho Clan",
                            "§l§cClan §f↣ §e/clan lookup : §axem người chơi thuộc Clan nào",
                            "§l§cClan §f↣ §e/clan requestlist : §axem danh sách các thành viên xin vào Clan",
                            "§l§cClan §f↣ §e/clan status : §axem thông tin Clan",
                            "§l§cClan §f↣ §e/clan invite : §amời thêm thành viên vào Clan",
                            "§l§cClan §f↣ §e/clan kick : §akick thành viên khỏi Clan",
                            "§l§cClan §f↣ §e/clan motd : §asửa Motd Clan",
                            "§l§cClan §f↣ §e/clan setrank : §aset chức vụ cho thành viên trong Clan",
                            "§l§cClan §f↣ §e/clan levelup : §anâng cấp Clan",
                        ];
                    ...
                    case "create":
                        ...
                        if (count($altArgs) <> 1) {
                            ...
                            $sender->sendMessage("§l§3• §cUsage: /clan create <clanName>\nLưu ý: Tạo Clan tốn $cost xu!");
                            return true;
                        }

                        if (strpos($altArgs[0], "§")) {
                            $sender->sendMessage("§l§3•§c Không được ghi màu trong lên clan.");
                            return true;
                        }

                        if (strlen($altArgs[0]) > 20) {
                            $sender->sendMessage("§l§3•§c Tên clan phải ngắn hơn 20 kí tự.");
                            return true;
                        }

                        if (isset($this->clans[$altArgs[0]])) {
                            $sender->sendMessage("§l§3•§c Clan đã tồn tại.");
                            return true;
                        }
                        ...
                        $cost = $this->getConfig()->get("create-cost");
                        if ($money < $cost) {
                            $sender->sendMessage("§l§3• §eBạn không có đủ tiền ($cost, bạn có $money)");
                            return true;
                        }
                        ...
                        $this->save();
                        $sender->sendMessage("§l§3• §eTạo Clan thành công!");
                        break;
                    case "invite":
                        ...
                        if (isset($altArgs[0])) switch ($altArgs[0]) {
                            case "accept":
                                if (isset($this->invitePending[$sender->getName()]) && $this->invitePending[$sender->getName()][0] > time()) {
                                    ...
                                    $sender->sendMessage("§l§3• §aLời mời không tồn tại.");
                                    return true;
                                }
                                break;
                            case "decline":
                                if (!isset($this->invitePending[$sender->getName()])) {
                                    $sender->sendMessage("§l§3• §aBạn không được bất cứ ai mời.");
                                    return true;
                                } else {
                                    $inviter = $this->getServer()->getPlayerExact($this->invitePending[$sender->getName()][2]);
                                    if ($inviter !== null) {
                                    ...
                                    unset($this->invitePending[$sender->getName()]);
                                    $sender->sendMessage("§l§3• §eĐã từ chối lời mời.");
                                    return true;
                                }
                                break;
                        }
                        ...
                        if ($profile->get("clan") === false || !$this->canDo($sender, self::ACTION_INVITE)) {
                            $sender->sendMessage("§l§3• §eBạn không có quyền mời người chơi vào clan.");
                            return true;
                        }
                        if ($this->isClanFull($profile->get("clan"))) {
                            $sender->sendMessage("§l§3• §eClan đã đầy.");
                            return true;
                        }
                        if (isset($altArgs[0])) switch ($altArgs[0]) {
                            case "yes":
                                if (isset($this->inviteSendConfirm[$sender->getName()]) && $this->inviteSendConfirm[$sender->getName()][0] > time()) {
                                    ...
                                    $sender->sendMessage("§l§3• §aLời mời đã được gửi.");
                                    $player->sendMessage("§l§3•§d " . $sender->getName() . " §6đã gửi cho bạn một lời mời vào clan§b '" . $profile->get("clan") . "'.");
                                    $player->sendMessage("§l§3• §bBấm §c/clan invite accept§a để đồng ý hoặc decline để từ chối.");
                                    $player->sendMessage("§l§3• §cLời mời sẽ tự động hủy sau 50 giây.");
                                    return true;
                                } else {
                                    $sender->sendMessage("§l§3• §aLời mời không tồn tại.");
                                    return true;
                            ...
                            case "no":
                                if (!isset($this->inviteSendConfirm[$sender->getName()])) {
                                    $sender->sendMessage("§l§3• §aBạn không mời bất cứ ai.");
                                    return true;
                                } else {
                                    unset($this->inviteSendConfirm[$sender->getName()]);
                                    $sender->sendMessage("§l§3• §aĐã hủy lời mời.");
                                    return true;
                                }
                                break;
                        }
                        if (count($altArgs) <> 1) {
                            $sender->sendMessage("§l§3• §cUsage: /clan invite <playerName>");
                            return true;
                        }
                        $player = $this->getServer()->getPlayer($altArgs[0]);
                        if ($player === null || $player->getName() === $sender->getName()) { // TODO this. is. bad.
                            $sender->sendMessage("§l§3• §eKhông tìm thấy người chơi nào với từ khóa '" . $altArgs[0] . "'");
                            return true;
                        } else if ($player->getName() === $altArgs[0] && $this->getProfile($player)->get("clan") !== false) {
                            $sender->sendMessage("§l§3• §eNgười chơi " . $altArgs[0] . " đã có clan rồi.");
                            return true;
                        }
                        if (strtolower($altArgs[0]) <> strtolower($player->getName())) {
                            $this->inviteSendConfirm[$sender->getName()] = [time() + 50, $player->getName()];
                            $sender->sendMessage("§l§3• §aChúng tôi không tìm thấy người chơi '" . $altArgs[0] . "'.\nCó phải bạn muốn mời '" . $player->getName() . "'? Gõ /clan invite <yes(có)|no(không)>");
                            $sender->sendMessage("§l§3• §aLời mời sẽ tự động hủy sau 50 giây.");
                            return true;
                        }
                        $this->invitePending[$player->getName()] = [time() + 50, $profile->get("clan"), $sender->getName()];
                        $sender->sendMessage("§l§3• §aLời mời đã được gửi.");
                    ...
                    case "status":
                        ...
                        $mes = "§l§3• §eClan§a " . $clan["motd"] . " (" . $profile->get("clan") . "):\n";
                        $mes .= "§l§3• Cấp Clan: §a" . $clan["level"] . "\n";
                        $mes .= "§l§3• Điểm Clan: §a" . $clan["point"] . "\n";
                        $mes .= "§l§3• Chức vụ của bạn: §a" . $this->getRankName($profile->get("rank")) . "\n";
                        $mes .= "§l§3• Thành Viên:§c\n";
                    ...
                    case "kick":
                        ...
                        if ($profile->get("clan") === false || !$this->canDo($sender, self::ACTION_KICK)) {
                            $sender->sendMessage("§l§3• §eBạn không có quyền kick người chơi khỏi clan.");
                            return true;
                        }
                        if (isset($altArgs[0])) switch ($altArgs[0]) {
                            case "yes":
                                if (isset($this->kickPending[$sender->getName()]) && $this->kickPending[$sender->getName()][0] > time()) {
                                    ...
                                    if ($targetProfile->get("clan") !== $profile->get("clan")) {
                                        $sender->sendMessage("§l§3• §aNgười này không ở trong Clan của bạn.");
                                        unset($this->kickPending[$sender->getName()]);
                                        $sender->sendMessage("§l§3• §aĐã hủy yêu cầu kick.");
                                        return true;
                                    }
                                    $result = $this->removePlayerFromClan($pName);
                                    unset($this->kickPending[$sender->getName()]);
                                    $sender->sendMessage($result ? "§l§3• §aĐã kick người chơi." : "Có lỗi xảy ra trong quá trình kick.");
                                    return true;
                                } else {
                            ...
                            case "no":
                                if (!isset($this->kickPending[$sender->getName()])) {
                                    $sender->sendMessage("§l§3• §aBạn không kick bất cứ ai.");
                                    return true;
                                } else {
                                    unset($this->kickPending[$sender->getName()]);
                        ...
                        if (count($altArgs) <> 1) {
                            $sender->sendMessage("§l§3• §cUsage: /clan kick <playerName>");
                            return true;
                        }
                        ...
                        if ($found === null) {
                            $sender->sendMessage("§l§3• §eKhông tìm thấy người chơi nào với từ khóa '$needle' trong clan của bạn.");
                            return true;
                        }
                        $this->kickPending[$sender->getName()] = [time() + 50, $profile->get("clan"), $found];
                        $sender->sendMessage("§l§3• §cBạn có chắc chắn muốn kick '$found' khỏi clan?");
                        $sender->sendMessage("§l§3• §cBấm /clan kick yes để xác nhận.");
                        $sender->sendMessage("§l§3• §cYêu cầu kick sẽ tự động hủy sau 50 giây.");
                        break;
                    case "requestlist":
                    case "rl":
                        if (!($sender instanceof Player)) {
