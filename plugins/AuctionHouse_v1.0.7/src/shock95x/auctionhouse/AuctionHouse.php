...
use shock95x\auctionhouse\libs\JackMD\ConfigUpdater\ConfigUpdater;
...
class AuctionHouse extends PluginBase {
	...
	public function sendAHMenu(Player $player, $currentMenu = null, int $n = 1) {
		...
		for($j = $n2; $j <= $n3; ++$j) {
			...
			$item->setCompoundTag($tag)->setCustomName(TextFormat::RESET . $item->getName() . "\n" . TextFormat::GRAY . str_repeat("§l-", 25) . "\n" . TextFormat::GREEN . "§l§6● §aBấm Vào Đây Để Mua.\n\n" . TextFormat::BLUE . "§l§6● §bGía: " . TextFormat::YELLOW . $this->economyProvider->getMonetaryUnit() . $auction->getPrice() . "\n" . TextFormat::BLUE . "§l§6● §bNgười Bán: " . TextFormat::YELLOW . $auction->getSeller() . "\n" . TextFormat::BLUE . "§l§6● §bHết Hạn: " . TextFormat::YELLOW . ($endTime->days * 24 + $endTime->h) . ":" . $endTime->i . "\n" . TextFormat::GRAY . str_repeat("§l-", 25));
	...
	public function sendExpired(Player $player, Inventory $currentMenu = null, int $n = 1) : void {
		...
		for($j = $n2; $j <= $n3; ++$j) {
			...
			$tag = $item->hasCompoundTag() ? $item->getNamedTag() : new CompoundTag();
			$tag->setLong("marketId", $auction->getMarketId());
			$item->setCompoundTag($tag)->setCustomName(TextFormat::RESET . $item->getName() . "\n" . TextFormat::GRAY . str_repeat("§l-", 25) . "\n" . TextFormat::GREEN . "§l§6● §aBấm Vào Đây Để Thoát.\n\n" . TextFormat::BLUE . "§l§6● §bGía: " . TextFormat::YELLOW . $this->economyProvider->getMonetaryUnit() . $auction->getPrice() . "\n" . TextFormat::GRAY . str_repeat("§l-", 25));
			$menu->getInventory($player)->addItem($item);
		}
		Pagination::setPage($player, $n);
		$total = count($list);
	...
	public function sendListings(Player $player, Inventory $currentMenu = null, int $n = 1) : void {
		...
		for($j = $n2; $j <= $n3; ++$j) {
			...
			$tag = $item->hasCompoundTag() ? $item->getNamedTag() : new CompoundTag();
			$tag->setLong("marketId", $auction->getMarketId());
			$item->setCompoundTag($tag)->setCustomName(TextFormat::RESET . $item->getName() . "\n" . TextFormat::GRAY . str_repeat("§l-", 25) . "\n" . TextFormat::RED . "§l§6● §aBấm Vào Đây Để Thoát.\n\n" . TextFormat::BLUE . "§l§6● §bGía: " . TextFormat::YELLOW . $this->economyProvider->getMonetaryUnit() . $auction->getPrice() . "\n" . TextFormat::GRAY . str_repeat("§l-", 25));
			$menu->getInventory($player)->addItem($item);
		}
		Pagination::setPage($player, $n);
		$total = count($list);
