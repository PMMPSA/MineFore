...
class Main extends PluginBase{
    ...
    */
    public function listForm(Player $player): void{
		...
		$form->setContent("§l§3●§e Tiền của bạn:§a ".$point." Points");
    ...
    */
    public function buyForm(Player $player,int $id): void{
        ...
        $form = $this->formapi->createCustomForm(function (Player $player, $data = null) use ($array, $id){
            ...
            "LEVEL" => $data[1],
            "MONEY" => $this->pointAPI->myMoney($player)
            );
            if ($data === null){
            ...
            if($this->pointAPI->myMoney($player) >= $c = $array[$id]['price'] * $data[1]){
                $this->pointAPI->reduceMoney($player, $c);
