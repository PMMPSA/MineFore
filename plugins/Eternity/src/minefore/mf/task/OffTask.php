			...
			foreach($this->getPlugin()->cons->getAll() as $p => $con){
				if($con == 0){
					...
					if($this->getPlugin()->getServer()->getPlayer($p) !== null){
						$this->getPlugin()->getServer()->getPlayer($p)->sendMessage("§l§6[§bSky§aBlock(Bin)§6] §cEternity§e của bạn đã hết hạn");
					}					
					continue;
				}
				if($this->getPlugin()->getServer()->getPlayer($p) !== null){
					$this->getPlugin()->getServer()->getPlayer($p)->sendMessage("§l§6[§bSky§aBlock(Bin)§6] §cEternity§e của bạn còn§a $con phút");
				}
