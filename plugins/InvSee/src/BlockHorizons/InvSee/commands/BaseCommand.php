...
abstract class BaseCommand extends Command implements PluginIdentifiableCommand {
	...
	public static function registerDefaults(Loader $loader): void {
		...
		foreach([
			EnderInvSeeCommand::class => ["enderinvsee", "Xem rương ender của người chơi.", "/enderinvsee <player>", "invsee.enderinventory.view"],
			InvSeeCommand::class => ["invsee", "Xem túi đồ của người chơi.", "/invsee <player>", "invsee.inventory.view"]
	...
	final public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
		...
		if($sender instanceof ConsoleCommandSender && $this->isFlagSet(self::FLAG_DENY_CONSOLE)) {
			$sender->sendMessage(TextFormat::RED . "§l§6[§bSky§aBlock(Bin)§6]§e Hãy sử dụng lệnh trong game.");
			return false;
		}
	...
	public function sendPermissionMessage(CommandSender $sender): void {
		$sender->sendMessage(TextFormat::RED . "§l§6[§bSky§aBlock(Bin)§6]§e Bạn không có quyền để sử dụng lệnh này.");
	}
