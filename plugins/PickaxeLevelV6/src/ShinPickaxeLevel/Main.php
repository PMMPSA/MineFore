...
class Main extends PluginBase implements Listener{
	...
	public function onItemHeld(PlayerItemHeldEvent $ev){
		...
		if ($it[0] == "§l§6【") {
			if ($damage > 780) {
				...
				$p->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§e Cúp đã được sửa chữa miễn phí!");
			}
			$popup = "   §l§e☛§d Pickaxe §6│ §bsκʏ§aʙʟocκ(ʙιɴ)§c §6☭\n" . "§c⚒§a Kinh Nghiệm:§b " . $exp ."§l§6 /§d ".$next. "§c |§6 Cấp Cúp: §2" . $level;
			$p->sendPopup($popup);
		}	
	...
	public function onBreak(BlockBreakEvent $ev){
		...
		if($pas[0] == "§l§6【"){
			if(strpos($icn, $name)  == false){
				...
				$p->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§e Vật phẩm không phải của bạn!");
				}
		}
		    ...
		    if($pas[0] == "§l§6【"){
				...
				if($this->getExp($p) >= $this->getNextExp($p)){
					...
					if(in_array($this->getLevel($p), [1000 ])){
						...
						$p->sendMessage("§l§c●§a Bạn được cộng§b $point point");
                    }
					...
					$this->getServer()->broadcastMessage("§l§6[§bSky§aBlock(Bin)§6]§d Cúp của người chơi§c ".$p->getName()."§d Vừa lên cấp§e : ".$this->getLevel($p));
			...
			if($cs > 1){
				...
				if($add > 1){
					...
					$p->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§a Bạn đã nhận được§d $add xu §btừ chuyển sinh cấp§c $cs");
				}
			}
	}
	...
	public function onCommand(CommandSender $sender, Command $cmd,string $label, array $args) : bool {
         if($cmd->getName() == "givecup"){
			 if($sender->isOp() || $sender->getName() == "NoobPvP2k3"){
                if(!isset($args[0])){
                    $sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§a /givecup <player>");
                    return true;
                }else{
                    ...
                    if(!$player == null){
                        if($player->isOnline()) {
                            ...
                            $inv->addItem($cup);
                            $player->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§e Cúp §bMine§aFore§e ...");		
                 ...
                 $sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§b You don't permission use command");
                 return true;
             }
		}elseif($cmd->getName() == "topcup"){
			 $this->sendTop($sender->getName(), $this->level->getAll());
		}elseif($cmd->getName() == "chuyensinh"){
			$tien = $this->eco->myMoney($sender);
			$amount = 20000000;
			$solan = $this->getRebirth($sender);
			if($sender->isOp()){
			   $sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§c Bạn là op đừng chiếm top của mem!");
			   return true;
			}
			if(!($tien >= $amount*$solan)){
				$sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§e Bạn không có đủ tiền để§a chuyển sinh! §eBạn cần§c ".$amount*$solan." xu");
			}else{
				$sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§e Bạn đã §achuyển sinh §ethành công! + 1 lần chuyển sinh");
			...
			$this->sendTop2($sender->getName(), $this->rb->getAll());
		}
	...
	public function levelUp(Player $player){
		...
		$this->data[$player->getName()]["exp"] = 0;
		$this->data[$player->getName()]["nextexp"] = ($this->getLevel($player)+1)*120;
	...
	public function nextExpUp(Player $player, $nextexp){
		$this->data[$player->getName()]["nextexp"] = $nextexp + 100;
	}	
	
	public function getNamePickaxe(Player $player){
		return "§l§6【 §bsκʏ§aʙʟocκ(ʙιɴ) §6】 §d". (string)$this->getLevel($player) ." §c➜§b ".$player->getName();
