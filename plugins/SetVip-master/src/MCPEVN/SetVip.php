...
class SetVip extends PluginBase implements Listener {
	...
	public $prefix = "§b§l§6[§bSky§aBlock(Bin)§6]§e";
	public $pp;
	...
	public function onJoin(PlayerJoinEvent $ev){
		...
		if($this->time->exists($p->getName())){
			...
			$i = explode(",", $st);	
			$this->getServer()->broadcastMessage("§l§6●§c Đại gia§d ".$p->getName()."§a " . $i[2] . "§b đã vào máy chủ!");
            ...
            if ($s->hasPermission("setvip.cmd")) {
                if (count($args) == 3) {
                    ...
                    if ($p instanceof \pocketmine\Player) {
						$p->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§a Bạn đã nhận được vip: §e". $rank ." §atrong ". $time ." ngày");
                    }
                    settype($name, "string");
					$s->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§6 Set VIP:§c ". $rank ." §athành công cho §b". $p->getName() ." §ctrong ". $time);
                    return true;
                }
    ...
    public function tick() {
        ...
        if (count($all) >= 1) {
            foreach ($all as $name => $stuff) {
				...
				$p->sendMessage("§l§6[§bSky§aBlock(Bin)§6]§a ". $i[2] ."§e Của bạn đã hết hạn, nếu muốn tiếp tục có vip, xin bạn hãy mua thêm!");
                    }
