...
class EventListener implements Listener{
	...
	public function onMove(PlayerMoveEvent $event) : void{
		...
		if(in_array($player->getName(), AfkCommand::$afk)){
			...
			$player->sendMessage("§l§6[§bSky§aBlock(Bin)§6] "   . TextFormat::GREEN . "You are no longer in afk mode");
		...
		if($entity instanceof Player){
			...
			if($entity->getPosition()->getY() < 0){
				if(API::getMainInstance()->getConfig()->get("novoid") === "on"){
					...
					$entity->sendMessage("§l§6[§bSky§aBlock(Bin)§6] "   . TextFormat::GREEN . "You were teleported out of the void");
				}
			}
		}
	}
