...
class GamemodeSpectatorCommand extends BaseCommand{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args) : bool{
		...
		if(empty($args[0])){
			...
			$sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6] "   . TextFormat::GREEN . "You have set your gamemode to spectator");
			return false;
		}
		if(API::getMainInstance()->getServer()->getPlayer($args[0])){
			$player = API::getMainInstance()->getServer()->getPlayer($args[0]);
			$player->setGamemode(Player::SPECTATOR);
			$player->sendMessage("§l§6[§bSky§aBlock(Bin)§6] "   . TextFormat::GREEN . "Your gamemode has been set to spectator");
			$sender->sendMessage("§l§6[§bSky§aBlock(Bin)§6] "   . TextFormat::GREEN . "You have set " . $player->getName() . "'s gamemode to spectator");
		}
		return true;
	}
}